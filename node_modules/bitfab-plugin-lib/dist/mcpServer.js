...
export function createMcpServer(platform, getConfig, pluginVersion) {
    ...
    server.registerTool("create_trace_plan", {
        description: "Post a tracing instrumentation plan and get back a URL the user opens to confirm or adjust. Send the full call tree (root + children + framework spans) the agent has analyzed plus initial recommended captured node ids and any sample input/output stored on the relevant nodes. Returns a URL the user opens in the browser; after they confirm or abort there, call get_trace_plan to read the final captured set and act on it.",
        inputSchema: {
            language: z
            ...
            tree: z
                ...
                .describe("TracePlanTree: { rootId, nodes: { [id]: TraceNode } }. Each TraceNode has id, name, kind ('manual' | 'auto' | 'pure'), file, line, signature, parentId, childIds, plus optional framework, fields, sampleInput, sampleOutput."),
            capturedNodeIds: z
                .preprocess(parseJsonString, z.array(z.string()))
                .describe("Initial recommended captured set. Must reference ids present in tree.nodes. Must form a connected sub-tree (selecting any descendant implies its ancestors)."),
            stats: z
                .preprocess(parseJsonString, z.unknown())
                .optional()
                .describe("Optional sample-run stats: { durationMs?, tokens?, cost? }"),
        },
    }, async ({ language, tree, capturedNodeIds, stats }) => proxyToolCall("create_trace_plan", {
        language,
        tree,
        capturedNodeIds,
        stats,
    ...
    server.registerTool("get_trace_plan", {
        description: "Read the current state of a trace plan. Use after creating a plan and asking the user to confirm in the browser, to learn whether they confirmed (apply edits using returned capturedNodeIds), cancelled (drop the plan and ask the user what to do next), or are still pending.",
        inputSchema: {
            planId: z
                .string()
                .describe("The trace plan id returned by create_trace_plan"),
        },
    }, async ({ planId }) => proxyToolCall("get_trace_plan", { planId }));
    return { server, proxy };
}
